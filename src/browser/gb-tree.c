...
#define GB_BROWSER_MENU_PATH "/org/gnome/Builder/data/ui/gb-tree-menu.ui"

G_DEFINE_TYPE(GbTree, gb_tree, GTK_TYPE_TREE_VIEW)
...
extern void _gb_tree_node_set_tree (GbTreeNode*, GbTree*);
               ...
               gint            target_y)
{
   ...
   menu = gtk_menu_new_from_model(priv->menu_model);
...
gb_tree_get_selected (GbTree *tree)
{
   ...
   if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
      ...
      g_object_unref(ret); /* Looks dangerous, it is not. */
   }
...
gb_tree_init (GbTree *tree)
{
   ...
   tree->priv = priv =
      G_TYPE_INSTANCE_GET_PRIVATE(tree,
                                  GB_TYPE_TREE,
                                  GbTreePrivate);
   ...
   g_object_set(tree, "model", priv->store, NULL);
   ...
   priv->menu_model = g_object_ref(gtk_builder_get_object(builder, "popup"));
