                     ...
                     GbTreeNode *child)
{
	...
	g_object_get(tree, "model", &model, NULL);
	g_assert(model);
                            ...
                            const gchar *icon_name)
{
	...
	priv->icon_name = g_quark_from_string(icon_name);
 ...
 *
 * Convenience property for storing a project item. Saves having to make
 * child classes for that.
 */
void
                       ...
                       GbProjectItem *item)
{
	...
	g_return_if_fail(GB_IS_TREE_NODE(node));
	g_return_if_fail(!item || GB_IS_PROJECT_ITEM(item));
 ...
 * @node: (in): A #GbTreeNode.
 *
 * Gets a #GbProjectItem for the node, if one was set.
 *
...
gb_tree_node_class_init (GbTreeNodeClass *klass)
{
	...
	g_type_class_add_private(object_class, sizeof(GbTreeNodePrivate));
    ...
    * GbTreeNode:item:
    *
    * A project item visualized by the node.
    */
...
{
	node->priv =
		G_TYPE_INSTANCE_GET_PRIVATE(node,
		                            GB_TYPE_TREE_NODE,
		                            GbTreeNodePrivate);
}
