...
static void
gb_workspace_layout_splash_child_activated (EggListBox              *list_box,
                                            GtkWidget               *row,
                                            GbWorkspaceLayoutSplash *splash)
{
   ...
   if (!(path = g_object_get_data(G_OBJECT(row), "path"))) {
...
static void
update_separators (GtkWidget **separator,
                   GtkWidget  *child,
                   GtkWidget  *before,
                   gpointer    user_data)
{
...
gb_workspace_layout_splash_init (GbWorkspaceLayoutSplash *splash)
{
   ...
   splash->priv->list_box = g_object_new(EGG_TYPE_LIST_BOX,
                                         "visible", TRUE,
                                         NULL);
   egg_list_box_set_separator_funcs(EGG_LIST_BOX(splash->priv->list_box),
                                    update_separators, NULL, NULL);
   egg_list_box_set_selection_mode(EGG_LIST_BOX(splash->priv->list_box),
                                   GTK_SELECTION_NONE);
   g_signal_connect(splash->priv->list_box,
                    "child-activated",
                    G_CALLBACK(gb_workspace_layout_splash_child_activated),
                    splash);
